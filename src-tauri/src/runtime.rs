...
/// 依赖 Windows 10 1803+ 内置的 curl；失败时尝试 ureq 回退，两段错误信息都并入返回值。
fn download_file(url: &str, dest: &Path) -> Result<(), String> {
    // 优先 curl（走系统代理），任一失败原因并入最终错误信息
    ...
    let last_err = match cmd.output() {
        ...
        Err(e) => format!("curl 启动失败: {e}"),
    };
    // 回退：ureq 直连
    let resp = ureq::get(url)
        .timeout(std::time::Duration::from_secs(120))
        .call()
        .map_err(|e| format!("{last_err}；ureq 回退也失败: {e}"))?;
